...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        attention_config = attention.MultiHeadAttentionConfig(
            num_heads=num_heads,
            hidden_size=embed_dim,
            attention_class="scaled_dot_product",
            attention_dropout_prob=dropout,
        )
        self.self_attention = attention.MultiHeadAttention.from_config(attention_config)
        ...
        if is_decoder:
            self.cross_attention = attention.MultiHeadAttention.from_config(attention_config)
