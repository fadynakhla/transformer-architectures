...
from ta.architectures.vanilla import data, tokenization
from ta.data import samplers
from ta.data.datasets import wmt_en_fr
from ta.training import data_utils, distributed
...
class VanillaDataModule(distributed.DataModule):
    ...
    def setup(self, ctx: distributed.DistributedContext) -> None:
        ...
        logger.info(
            f"Creating local train dataset on rank {ctx.world_rank} with {len(local_train)} samples."
        )
